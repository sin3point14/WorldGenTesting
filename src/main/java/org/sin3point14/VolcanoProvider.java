...
public class VolcanoProvider implements FacetProvider {
    ...
    public void process(GeneratingRegion region) {
        Border3D border = region.getBorderForFacet(VolcanoFacet.class).extendBy(0, Volcano.MAXHEIGHT, Volcano.MAXWIDTH);
...
//                    if (noise.noise(wx, wz) > 0.9999 && checkGradient(new Vector3i(wx, surfaceHeight, wz), surfaceHeightFacet)) {
...
//        noise = new SubSampledNoise(new WhiteNoise(seed), new Vector2f(0.1f, 0.1f), Integer.MAX_VALUE);
    ...
    private boolean checkGradient(Vector3i corner, BaseFieldFacet2D facet) {
